...
ALTER TABLE platby ADD CONSTRAINT platby_pk PRIMARY KEY ( id_platby );

CREATE TABLE platby_kartou (
    id_platby      NUMBER(32) NOT NULL,
    cislo_karty    VARCHAR2(16) NOT NULL,
    jmeno_majitele VARCHAR2(100) NOT NULL
);

ALTER TABLE platby_kartou ADD CONSTRAINT platba_kartou_pk PRIMARY KEY ( id_platby );

CREATE TABLE platby_prevodem (
    id_platby  NUMBER(32) NOT NULL,
    cislo_uctu VARCHAR2(20) NOT NULL
);

ALTER TABLE platby_prevodem ADD CONSTRAINT platba_prevodem_pk PRIMARY KEY ( id_platby );
...
CREATE TABLE zamestnanci (
    ...
    id_zamestnance1    NUMBER(32) NOT NULL,
    id_uzivatele       NUMBER(32) NOT NULL
);
...
ALTER TABLE jizdy_ridicu
    ADD CONSTRAINT jizda_fk FOREIGN KEY ( id_jizdy )
...
ALTER TABLE platby_kartou
    ADD CONSTRAINT platba_kartou_platby_fk FOREIGN KEY ( id_platby )
        REFERENCES platby ( id_platby );

ALTER TABLE platby_prevodem
    ADD CONSTRAINT platba_prevodem_platby_fk FOREIGN KEY ( id_platby )
        REFERENCES platby ( id_platby );

ALTER TABLE platby
    ADD CONSTRAINT platby_zakaznik_fk FOREIGN KEY ( id_zakaznika )
        REFERENCES zakaznici ( id_zakaznika );
...
ALTER TABLE jizdy_ridicu
    ADD CONSTRAINT ridic_fk FOREIGN KEY ( id_ridice )
...
ALTER TABLE zamestnanci
    ADD CONSTRAINT zamestnanec_zamestnanec_fk FOREIGN KEY ( id_zamestnance1 )
...
CREATE OR REPLACE TRIGGER arc_fkarc_1_platby_kartou BEFORE
    INSERT OR UPDATE OF id_platby ON platby_kartou
    FOR EACH ROW
DECLARE
    d NUMBER(1);
BEGIN
    ...
    FROM
        platby a
    WHERE
        a.id_platby = :new.id_platby;

    IF ( d IS NULL OR d <> 0 ) THEN
        raise_application_error(-20223, 'FK PLATBA_KARTOU_PLATBY_FK in Table PLATBY_KARTOU violates Arc constraint on Table PLATBY - discriminator column typ_platby doesn''t have value 0'
        );
    END IF;
...
CREATE OR REPLACE TRIGGER arc_fkarc_1_platby_prevodem BEFORE
    INSERT OR UPDATE OF id_platby ON platby_prevodem
    FOR EACH ROW
DECLARE
    d NUMBER(1);
BEGIN
    ...
    IF ( d IS NULL OR d <> 1 ) THEN
        raise_application_error(-20223, 'FK PLATBA_PREVODEM_PLATBY_FK in Table PLATBY_PREVODEM violates Arc constraint on Table PLATBY - discriminator column typ_platby doesn''t have value 1'
        );
    END IF;
